...
from algos.replay_buffer import replay_buffer, replay_buffer_energy, DynamicReplayBuffer, SelfImitationReplayBuffer
...
from algos.adjacency_net import ANet, update_amat_and_train_anet
import gym
import pickle
from planner.simhash import HashingBonusEvaluator
from planner.grid_hash import GridHashing
from planner.repr_adj_grid import ReprAdjGrid
...
class hier_sac_agent:
    def __init__(self, args, env, env_params, test_env, test_env1=None, test_env2=None):
        ...
        # zyb add dyn_interval
        self.dyn_interval = 100
        ...
        # record all history subgoal (False)
        ...
        # her sampler
        self.c = self.args.c  # interval of high level action (H)
        ...
        # replay buffer with specified sampler
        if args.env_name[:5] == "Fetch":
        ...
        '''
        additional variables are all added below
        '''
        self.repr_grid_scale = 2
        self.dist_hash = ReprAdjGrid(scale=self.repr_grid_scale)
        self.repr_traj_buffer = TrajectoryReprBuffer()
        self.update_grid_path = True
        self.episode_start = None
        self.self_imitation_buffer = SelfImitationReplayBuffer()
        
        # below are debug variables
        self.__debug_high_self_imitation = True
        print("init end")
    ...
    # train the agent
    def learn(self):
        print("Max Timesteps: ", self.env_params['max_timesteps'])
        cum_timestep = 0
        # one 'epoch' is one 'episode' (start_epoch is zeros at the beginning)
        for epoch in range(self.start_epoch, self.args.n_epochs):
            frontier_set = False  # 是否被选择过作为特殊episode，整个回合内不变
            is_frontier_reaching = False  # 当前是否作为特殊回合对待，可能改变
            self.route = None
            random_explore = False
            prev_ag, prev2_ag = None, None
            ...
            if epoch > (self.start_count_success - 200) and np.random.rand() < frontier_prop:
                # 这个episode的任务改为导航到距离起点最远点后随机探索
                is_frontier_reaching = True
                frontier_set = True
                print("frontier reaching episode")
            
            if epoch % 10 == 0:
                print('epoch: ', epoch)
            if epoch > 0 and epoch % self.args.lr_decay_actor == 0:  # 3000
            ...
            self.repr_traj_buffer.create_new_trajectory()
            self.repr_traj_buffer.append(list(obs))
            ...
            if is_frontier_reaching:
                epi_start = ag
                frontier = np.array(self.dist_hash.query_max_dist_from(epi_start))  # (2,) in subgoal space
            ...
            for t in range(self.env_params['max_timesteps']):
                ...
                if t % self.c == 0:
                    ...
                    if not is_frontier_reaching:
                        # select subgoal by counts, the subgoal is absolute subgoal
                        # high action selected by 'select_by_count' method is deemed as an absolute position
                        if self.count_latent and epoch > self.start_explore and valid_count and (self.intrinsic_coeff == 0.):
                            ...
                            if self.old_sample:
                                ...
                                # record subgoal selected by intrinsic rewards (False)
                                if self.intrinsic_coeff > 0.:
                                    current_hi_step = int(t / self.c)
                                    self.subgoal_record[current_hi_step].append(hi_action_for_low)
                            hi_action_delta = hi_action
                    else:
                        if np.linalg.norm(ag - frontier) < 8:
                            random_explore = True
                        elif (prev_ag is not None and prev2_ag is not None and np.linalg.norm(ag - prev_ag) < 1 and np.linalg.norm(ag - prev2_ag) < 1):
                            random_explore = True
                            is_frontier_reaching = False
                        
                        if not random_explore:
                            _waypoint = self.select_waypoint(ag, epi_start, frontier)  # cur, start, end
                            if _waypoint is not None:
                                hi_action_ini = _waypoint
                            else: 
                                hi_action_ini = frontier
                        else:
                            frontier_num_random_samples = 100
                            hi_actions_candidates = ag + np.random.uniform(low=np.array([-self.abs_range, -self.abs_range]), high=np.array([self.abs_range, self.abs_range]), size=(frontier_num_random_samples, self.real_goal_dim))
                            random_count_score = np.array(self.future_hash.predict(hi_actions_candidates))
                            random_min_index = random_count_score.argmin()
                            hi_action_ini = hi_actions_candidates[random_min_index]
                            
                        prev2_ag = prev_ag.copy() if (prev_ag is not None) else None
                        prev_ag = ag.copy()
                    ...
                    # update high-level policy (high actions used for training are [delta] repr positions, not absolute repr positions)
                    # but high actions supplied to low policy are absolute repr positions
                    if len(self.hi_buffer) > self.args.batch_size and self.learn_hi:
                        self.update_hi(epoch, t)
                # #################################### high policy select action end ########################################
                
                # low policy select action
                with torch.no_grad():
                    if self.not_train_low:
                        action = self.test_policy(act_obs[:, :self.low_dim], hi_action_tensor)
                    else:
                        if is_frontier_reaching and not random_explore:
                            action = self.deterministic_policy(act_obs[:, :self.low_dim], hi_action_tensor)
                        else:
                ...
                # zyb add self_imitation_buffer data collection
                # TODO 之前不知道为啥写了个if not frontier_set才调用add，实在想不起来当时为啥这么写了，留个todo看看之后能不能想起来
                self.self_imitation_buffer.add(start_state, obs[:self.hi_dim], g)
            ...
            # ep_ag: shape(T, 2), mb_ag: shape(1, T, 2)
            ...
            # update high-level policy by self-imitation
            if epoch > self.start_count_success - 100:
                self.update_hi_self_imitation_batch(epoch)
                if not self.not_train_low:
                    self.update_low_self_imitation_batch(epoch)
            ...
            if epoch % self.args.eval_interval == 0 and epoch != 0:
                ...
                print('[{}] epoch is: {}, eval hier success rate is: {:.3f}  {:.3f}'.format(datetime.now(), epoch, random_success_rate, farthest_success_rate))
                record_test_sr(filepath='sr_log_' + f'{self.args.seed}' + '_' + str(self.args.env_name) + '.txt', epoch=epoch, t=cum_timestep, sr=farthest_success_rate)
                if self.save_fig:
            ...
            if epoch > self.start_update_phi and not self.not_update_phi and epoch % self.phi_interval == 0:
                ...
                # update hash table for history subgoal after updating phi (False)
            ...
            # instantiate a new Hashmap and dump all historical data into it (False)
            ...
            if epoch > self.start_update_phi and not self.not_update_phi and epoch % self.phi_interval == 0:
                traj_hist = self.repr_traj_buffer.get_trajectory()
                repr_list, repr_array = self.transform_data_to_repr(traj_hist)
                # after updating phi, we need to update path grid hash table too
                # instantiate a new Hashmap and dump the reprs into the new Hashmap
                if self.update_grid_path:
                    self.dist_hash = ReprAdjGrid(scale=self.repr_grid_scale, data=repr_list)
    ...
    def select_waypoint(self, cur, start, end):
        # 用self.route维护这个episode的期望路径，分摊时间复杂度
        # 在route中找一个最近的，如果距离较大就先选最近的，距离小的话就选最近的点再之后self.c步的点
        if start is None or end is None:
            raise ValueError("illegal start or end point")
        if self.route is None:
            self.route = self.dist_hash.query_route(start, end)  # (route_length, 2)
        if self.route.shape[0] == 0:
            return None
        dist_to_route = np.linalg.norm(self.route - cur, axis=-1)
        nearest_idx = np.argmin(dist_to_route)
        wp = self.route[min(nearest_idx + random.randint(self.c // 5, self.c // 3), self.route.shape[0]-1)] \
            + np.random.normal(loc=np.array([0, 0]), scale=SELF_IMITATION_NOISE_SCALE*np.array([1, 1]))
        return wp
    
    def transform_data_to_repr(self, obs_traj):
        steps_list = []
        obs_gather_list = []
        for traj in obs_traj:
            steps_list.append(len(traj))
            for obs in traj:
                obs_gather_list.append(obs)
        obs_gather_tensor = torch.FloatTensor(obs_gather_list).to(self.device)
        repr_gather_array = self.representation(obs_gather_tensor).detach().cpu().numpy()  # inference only once to speed up
        repr_gather_list = [list(item) for item in list(repr_gather_array)]  # List[List]
                    
        repr_list = []
        cur = 0
        for step in steps_list:
            repr_list.append(repr_gather_list[cur: cur+step])
            cur += step
                    
        return repr_list, repr_gather_array
    ...
    def __select_target(self, route):
        rand_idx = random.randint(self.c // 6, self.c // 4)
        select_idx = min(rand_idx, (len(route) - 1))
        return select_idx, route[select_idx].reshape(self.real_goal_dim)
    
    def update_hi_self_imitation_batch(self, epoch):
        if self.__debug_high_self_imitation:
            print("update_hi_self_imitation invoked")
            self.__debug_high_self_imitation = False
        starts_batch, obs_batch, goal_batch = self.self_imitation_buffer.sample(self.args.batch_size)  # (batch, obs_dim), (batch, obs_dim), (batch, 2)
        _goal_batch = np.concatenate([goal_batch, obs_batch[:, self.real_goal_dim:]], axis=-1)  # (batch, obs_dim+2)
        goal_repr = self.representation(torch.Tensor(_goal_batch).to(self.device)).detach().cpu().numpy()  # (batch, 2)
        obs_repr = self.representation(torch.Tensor(obs_batch).to(self.device)).detach().cpu().numpy()  # (batch, 2)
        starts_repr = self.representation(torch.Tensor(starts_batch).to(self.device)).detach().cpu().numpy()  # (batch, 2)
        
        routes = self.dist_hash.query_route_parallel(obs_repr, goal_repr)
        post_proc_idx = [k for k in range(len(routes)) if routes[k].shape[0] == 0]
        post_proc_pseudo_goal_repr = np.empty((len(post_proc_idx), self.real_goal_dim))
        post_proc_obs_repr = obs_repr[post_proc_idx, :]
        for idx in range(len(post_proc_idx)):
            post_proc_pseudo_goal_repr[idx, :] = np.array(self.dist_hash.query_max_dist_from(starts_repr[post_proc_idx[idx]]))  # (2,) in subgoal space
        post_proc_routes = self.dist_hash.query_route_parallel(post_proc_obs_repr, post_proc_pseudo_goal_repr)
        for idx in range(len(post_proc_idx)):
            routes[post_proc_idx[idx]] = post_proc_routes[idx]
        # -------------------------------------------------------------
        valid_idx = [i for i in range(len(routes)) if routes[i].shape[0] > 1]
        valid_obs_batch, valid_goal_batch = obs_batch[valid_idx, :], goal_batch[valid_idx, :]  # (valid_batchsize, obs_dim & 2)
        valid_obs_repr = obs_repr[valid_idx, :]  # (valid_batchsize, 2)
        valid_targets = np.array([self.__select_target(r)[1] for r in routes if r.shape[0] > 1]) - valid_obs_repr  # (valid_batchsize, 2)
        _target_noise = np.random.normal(loc=np.zeros(valid_targets.shape), scale=SELF_IMITATION_NOISE_SCALE*np.ones(valid_targets.shape))
        valid_targets = valid_targets + _target_noise
        target_tensor = torch.tensor(valid_targets).float().to(self.device)
        hi_input_tensor = torch.tensor(np.concatenate([valid_obs_batch[:, :self.hi_dim], valid_goal_batch], axis=-1)).float().to(self.device)
        
        if hi_input_tensor.shape[0] == 0:
            print("no legal data, ret (high)")
            return
        
        hi_action_batch, _, _ = self.hi_agent.policy.sample(hi_input_tensor)
        hi_imitation_loss = 0.01 * ((hi_action_batch - target_tensor) ** 2).mean()
        
        self.hi_agent.policy_optim.zero_grad()
        hi_imitation_loss.backward()
        self.hi_agent.policy_optim.step()
    ...
    def update_low_self_imitation_batch(self, epoch):
        # print("updating low policy by self-imitation")
        self.low_actor_network.train()
        num_refs_per_sample = int(self.c // 6)  # num of nearer subgoals for reference
        
        starts_batch, obs_batch, goal_batch = self.self_imitation_buffer.sample(batchsize=self.args.batch_size)
        _goal_batch = np.concatenate([goal_batch, obs_batch[:, self.real_goal_dim:]], axis=-1)  # (batch, obs_dim+2)
        goal_repr = self.representation(torch.Tensor(_goal_batch).to(self.device)).detach().cpu().numpy()  # (batch, 2)
        obs_repr = self.representation(torch.Tensor(obs_batch).to(self.device)).detach().cpu().numpy()  # (batch, 2)
        starts_repr = self.representation(torch.Tensor(starts_batch).to(self.device)).detach().cpu().numpy()  # (batch, 2)
        ...
        for idx in range(len(post_proc_idx)):
            routes[post_proc_idx[idx]] = post_proc_routes[idx]
        # ---------------------------------------------------------
        valid_idx = [i for i in range(len(routes)) if routes[i].shape[0] > 2]
        valid_obs_batch, valid_goal_batch = obs_batch[valid_idx, :], goal_batch[valid_idx, :]  # (valid_batchsize, obs_dim & 2)
        valid_obs_repr = obs_repr[valid_idx, :]  # (valid_batchsize, 2)
        select_info = [self.__select_target(r) for r in routes if r.shape[0] > 2]
        # print("select info: ", select_info)
        valid_hi_action_for_low = np.array([s[1] for s in select_info])  # (valid_batchsize, 2)
        valid_hi_action_for_low = valid_hi_action_for_low + np.random.normal(loc=np.zeros(valid_hi_action_for_low.shape), scale=SELF_IMITATION_NOISE_SCALE*np.ones(valid_hi_action_for_low.shape))
        valid_obs_for_low = np.concatenate([valid_obs_batch, valid_obs_repr], axis=-1)
        
        # 没有合法数据就直接退出
        if valid_obs_for_low.shape[0] == 0:
            print("no legal data, ret (low)")
            return
        
        # 基于待对齐子目标选出的动作，(batchsize, act_dim)
        action_to_align = self.low_actor_network(
            torch.Tensor(valid_obs_for_low).to(self.device), torch.Tensor(valid_hi_action_for_low).to(self.device))
        action_to_align = action_to_align.unsqueeze(1).repeat(1, num_refs_per_sample, 1)  # (batchsize, num_refs, act_dim)
        
        valid_hi_action_refs = np.empty((len(valid_idx), num_refs_per_sample, self.real_goal_dim))
        _cur = 0
        for i in range(len(routes)):
            r = routes[i]
            if r.shape[0] <= 2:
                continue
            # print("loop: ", _cur, len(r), len(select_info), select_info)
            refs_idx = np.linspace(1, max(1, int(min((len(r)-1), select_info[_cur][0]) // 2)), num_refs_per_sample).astype(int)
            valid_hi_action_refs[_cur, :, :] = r[refs_idx, :]
            _cur += 1
        
        # 基于参考目标选出的动作，(batchsize, num_refs, act_dim)
        with torch.no_grad():
            valid_hi_action_refs = valid_hi_action_refs + np.random.normal(loc=np.zeros(valid_hi_action_refs.shape), scale=0.2*SELF_IMITATION_NOISE_SCALE*np.ones(valid_hi_action_refs.shape))
            valid_obs_for_low_tensor = torch.Tensor(valid_obs_for_low).float().unsqueeze(1).repeat(1, num_refs_per_sample, 1).to(self.device)
            valid_hi_action_refs_tensor = torch.Tensor(valid_hi_action_refs).float().to(self.device)
            # print("shape check: ", valid_obs_for_low_tensor.shape, valid_hi_action_refs_tensor.shape)
            # (batchsize, num_refs_per_sample, act_dim)
            target_actions = self.low_actor_network(valid_obs_for_low_tensor, valid_hi_action_refs_tensor)
        
        imitation_loss = 0.001 * ((action_to_align - target_actions) ** 2).mean() 
    ...
    def _update_network(self, epoch, buffer, actor_target, critic_target, actor, critic, T, actor_optim, critic_optim,
        ...
        critic_loss = critic_loss + 0. * L2_reg
    ...
    def slow_update_phi(self, epoch, update_i):
        ...
        if not self.args.use_prediction:
            ...
            ini_representation_loss = ini_representation_loss + self.feature_reg * (obs / self.abs_range).pow(2).mean()
        else:
    ...
    def select_by_count(self, hi_obs, t, epoch):
        ...
        near_indexs = np.where((distances < max(20, self.abs_range)) & (distances > self.min_dist))[0]
        ...
        if len(count) == 0:
            ...
            components of score:
            1. (future) count
            2. distance from current state to subgoal candidates
            3. success count
            '''
    ...
    def _preproc_obs(self, obs: torch.Tensor):
        # obs = torch.tensor(obs, dtype=torch.float32).unsqueeze(0).to(self.device)
        ag = self.representation(obs)
        obs = torch.cat([obs, ag], dim=1)
        return obs
...
def record_test_sr(filepath: str = 'sr_log.txt', epoch: int = 0, t: int = 0, sr: float = 0.):
    with open(filepath, 'a') as f:
        f.write(str(epoch) + ' ' + str(t) + ' ' + str(sr))
        f.write('\n')

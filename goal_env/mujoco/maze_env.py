...
class MazeEnv(gym.Env):
    ...
    def reset(self, goal):
        ...
        # debug: 生成临时文件放这里
        if not hasattr(self, "reset_xml_file"):
            _, file_path = tempfile.mkstemp(text=True, suffix='.xml')
            self.reset_xml_file = file_path
        ...
        if self.visualize_goal:  # remove the prev goal and add a new goal
            ...
            for geom in geoms:
                if 'name' not in geom.attrib:
                    raise Exception("Every geom of the torso must have a name "
                                    "defined")
            # debug: 这里使用同一个文件，不再生成新文件
            # _, file_path = tempfile.mkstemp(text=True, suffix='.xml')
            file_path = self.reset_xml_file
            # print(file_path)
            self.tree.write(
        ...
        if self.visualize_goal:  # remove the prev goal and add a new goal
            ...
            for geom in geoms:
                if 'name' not in geom.attrib:
                    raise Exception("Every geom of the torso must have a name "
                                    "defined")
            # 这里debug一下，原本是_, file_path, 改成file_path, _
            _, file_path = tempfile.mkstemp(text=True, suffix='.xml')
            self.tree.write(
                file_path)  # here we write a temporal file with the robot specifications. Why not the original one??
